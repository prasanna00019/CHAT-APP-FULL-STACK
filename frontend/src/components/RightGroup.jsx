...
import Message from './Message'; // Separate message component
...
const RightGroup = ({ clickedGroupId, setClickedGroupId, groups, userId,lastMessage,setlastMessage }) => {
  ...
  useEffect(() => {
    ...
    socket.on('messageDeletedForMe', (data) => {
      ...
      //   ...prevLastMessage,
      //   [data.group]: messages[messages.length-1]?.text,
      // }))  
    });
   
    // Handle 'messageDeletedForEveryone' event to update the UI for all users
    socket.on('messageDeletedForEveryone', (deletedMessage) => {
      if(deletedMessage.deletedForEveryone){
        ...
        const index = messages.findIndex((message) => message._id === deletedMessage._id);
console.log(index);
  console.log(index===messages.length-1);
      ...
      // setlastMessage((prevLastMessage) => ({
      //   ...prevLastMessage,
      //   [deletedMessage.group]: 'DELETED FOR EVERYONE',
      // }));
    }
    });
    // RightGroup.jsx
...
// socket.on('messageStarredGroup',(starredMessage)=>{
//   setMessages((prevMessages)=>prevMessages.map((msg) => msg._id === starredMessage._id ? starredMessage : msg));
// })
socket.on('messageEditedGroup', (updatedMessage) => {
  ...
  const handleTyping = () => {
    ...
    // Clear the timeout and set a new one for 'stopTypingGroup'
  ...
  const handleSendMessage = async () => {
    ...
    const messageData = {
      ...
      sender: userId, // Replace with the logged-in user ID
    };

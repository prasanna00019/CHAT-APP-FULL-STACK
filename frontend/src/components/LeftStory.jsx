...
const LeftStory = () => {
  ...
  const fetchStories = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5000/story/all/${Authuser._id}`,{
        method: 'GET',
        headers: {
...
//     username: Authuser.username,
//     content: newStoryContent,
//     visibility: ['670fe2280ad0e102b1d42256'],
//   }
// );
...
// };
  ...
  return (
    <div className='p-3 border border-gray shadow-2xl mt-[-20px] font-bold shadow-blue-400 h-full w-[300px] bg-white rounded-lg'>
      ...
      />
      <button onClick={()=>{createStory();sets1(!s1)}} className='bg-blue-500 text-white p-2 rounded'>
        Create Story
      </button>
      <hr />
